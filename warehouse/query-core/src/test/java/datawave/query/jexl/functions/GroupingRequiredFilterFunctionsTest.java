...
public class GroupingRequiredFilterFunctionsTest {
    ...
    public void testAtomValuesMatch() {
        ...
        Assert.assertTrue(GroupingRequiredFilterFunctions.atomValuesMatch(makeValueTuple("ALPHA.1,BAR,bar"), makeValueTuple("BETA.1,BAR,bar"),
        ...
        Assert.assertTrue(GroupingRequiredFilterFunctions.atomValuesMatch(makeValueTuple("ALPHA.1,BAR,bar"), makeValueTuple("BETA.2,BAR,bar"), // different
                                                                                                                                               // grouping
                                                                                                                                               // context
                        makeValueTuple("GAMMA.1,BAR,bar")).size() == 0);
        
        Assert.assertTrue(GroupingRequiredFilterFunctions.atomValuesMatch(Collections.singleton(makeValueTuple("ALPHA.1,BAR,bar")),
                        Collections.singleton(makeValueTuple("BETA.1,BAR,bar")), Collections.singleton(makeValueTuple("GAMMA.1,BAR,bar"))).size() == 3);
        
        Assert.assertTrue(GroupingRequiredFilterFunctions.atomValuesMatch(Collections.singleton(makeValueTuple("ALPHA.1,BAR,bar")),
                        Collections.singleton(makeValueTuple("BETA.1,BAR,bar")), Collections.singleton(makeValueTuple("GAMMA.1,BAZ,baz"))).size() == 0);
    ...
    public void testAtomValuesMatchMult() {
        ...
        Collection<?> matches = GroupingRequiredFilterFunctions.atomValuesMatch(
                Lists.newArrayList(makeValueTuple("ALPHA.1,BAR,bar"), makeValueTuple("ALPHA.2,BAZ,baz")),
                Lists.newArrayList(makeValueTuple("BETA.1,BAR,bar"), makeValueTuple("BETA.2,BAZ,baz"))
        );
        ...
        matches = GroupingRequiredFilterFunctions.atomValuesMatch(
                Lists.newArrayList(makeValueTuple("ALPHA.1,BAR,bar"), makeValueTuple("ALPHA.2,BAZ,baz")),
                Lists.newArrayList(makeValueTuple("BETA.1,BAR,bar"), makeValueTuple("BETA.2,BAZ,baz")),
                Lists.newArrayList(makeValueTuple("GAMMA.1,BAR,bar"), makeValueTuple("GAMMA.2,BAZ,baz"))
        );
    ...
    public void testMatchesInGroup() {
        // @formatter: off
        Assert.assertTrue(GroupingRequiredFilterFunctions.matchesInGroup(makeValueTuple("ALPHA.1,BAR,bar"), "bar", makeValueTuple("GAMMA.1,BAZ,baz"), "baz")
                        .size() == 2);
        
        Assert.assertTrue(GroupingRequiredFilterFunctions.matchesInGroup(Collections.singleton(makeValueTuple("ALPHA.1,BAR,bar")), "bar",
    ...
    public void testGetGroupsForMatchesInGroup() {
        ...
        Collection<?> groups = GroupingRequiredFilterFunctions.getGroupsForMatchesInGroup(makeValueTuple("ALPHA.1,BAR,bar"), "bar",
                        makeValueTuple("GAMMA.1,BAZ,baz"), "baz");
        Assert.assertTrue(groups.size() == 1);
        Assert.assertTrue(groups.contains("1"));
        
        // there is only one match and it is in context group .1
        groups = GroupingRequiredFilterFunctions.getGroupsForMatchesInGroup(Collections.singleton(makeValueTuple("ALPHA.1,BAR,bar")), "bar",
        ...
        groups = GroupingRequiredFilterFunctions.getGroupsForMatchesInGroup(
                        Lists.newArrayList(makeValueTuple("ALPHA.1,BAR,bar"), makeValueTuple("ALPHA.2,BAR,bar")), "bar",
                        Collections.singleton(makeValueTuple("GAMMA.1,BAZ,baz")), "baz");
        Assert.assertTrue(groups.size() == 1);
        Assert.assertTrue(groups.contains("1"));
        // @formatter: off
    ...
    public void testMatchesInGroupLeft() {
        ...
        groups = GroupingRequiredFilterFunctions.matchesInGroupLeft(Lists.newArrayList(makeValueTuple("NAME.grandparent_0.parent_0.child_1,FREDO,fredo"),
